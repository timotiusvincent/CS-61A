...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        for _ in range(self.min_range - 1):
            place = place.entrance
        while not place is hive and current_place < self.max_range:
            if place.bees:
...
class ShortThrower(ThrowerAnt):
    ...
    """def nearest_bee(self, hive):
        place = self.place
        while place.bees == [] and place.entrance is not hive:
            place = place.entrance
            self.range += 1
...
class LongThrower(ThrowerAnt):
    ...
    """def nearest_bee(self, hive):
        place = self.place
        while place.entrance is not hive and (self.range < self.min_range or place.bees == []):
            place = place.entrance
            self.range += 1
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing each turn but it has a large armor value to block
    off incoming bees."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is identical to a ThrowerAnt but it is watersafe and the
    armor wouldn't be reduced to 0 when in water."""
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.queen_instantiated:
            self.true_queen = False
        QueenAnt.queen_instantiated = True
        # END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.true_queen:
            ...
            while place:
                if place.ant:
                    if isinstance(place.ant, BodyguardAnt) and place.ant.contained_ant not in self.doubled_damage and place.ant.contained_ant:
                        self.doubled_damage += [place.ant.contained_ant]
                        place.ant.contained_ant.damage *= 2
                    if place.ant not in self.doubled_damage:
                        self.doubled_damage += [place.ant]
                        place.ant.damage *= 2
                place = place.exit
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    "*** YOUR CODE HERE ***"
    action = bee.action
    action_effect = effect(action, bee)

    def new_effect(colony):
        nonlocal duration
        if duration > 0:
            action_effect(colony)
            duration -= 1
        else:
            bee.direction = 1
            action(colony)
