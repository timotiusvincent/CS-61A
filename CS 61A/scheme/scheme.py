...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(proc, MacroProcedure):
            ...
            return scheme_apply(proc, rest.map(lambda x: scheme_eval(x, env)),
                                env)
        # END PROBLEM 5
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        "*** YOUR CODE HERE ***"
        frame = Frame(self.env).make_child_frame(self.formals, args)
        return frame
        # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        parameter, value = bindings.first.first, scheme_eval(bindings.first.second.first, env)
        args, val = Pair(parameter, args), Pair(value, val)
        bindings = bindings.second
    check_formals(args)
    return env.make_child_frame(args, val)
    # END PROBLEM 15
